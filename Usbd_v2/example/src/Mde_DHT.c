...
int DHT_Mde(void){
	 ...
	 uint8_t Humestring[6];
	uint8_t Tempstring[7];
	 uint8_t	aux_string[5];
...
//		Humestring[4] = ' ';
		...
		//saco los valores de la cola y los vuelvo a escribir para tener simpre el valor actual
//		xQueueReceive(actual_temp,&aux_string,0);
//		xQueueReceive(actual_hum,&aux_string,0);
		...
		return 1; //termino bien la lectura;
		break;
	}
